function SequentialChasteMovieTest()
cd('F:\ChasteVideos\GhostSizeTest\GhostCell2');
csvfiles = dir('*Fixed*.csv');
...
finalFile = csvfiles(end);
h = readtable(finalFile.name);
cell_parameters = cell(height(h),3);
...
for file = csvfiles'
    ...
    % Generate a new shape
        ...
        cell_parameters{n,1} = cellIndex;
        cell_parameters{n,2} = real(nuc_shape);
        cell_parameters{n,3} = imag(nuc_shape);
        
        n = n+1;
        
    else        
        cell_mean = cell_parameters(cat(1,cell_parameters{:,1}) == cellIndex,:);
        
        if (isempty(cell_mean))
            continue
        end
                
        nuc_shape = (cell_mean{2} + 0.5*real_std.*randn(size(real_std)))+1j*(cell_mean{3} + 0.5*imag_std.*randn(size(imag_std)));
        nuc_shape(5:end-4) = 0;
   end
